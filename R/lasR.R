...
las = function(las_file){
  ...
  if (length(sections$las_version) > 0) {
    ...
    stop(paste('Version Information Section not recognized in file', lasfile))
  }
  ...
  if (length(sections$well) > 0) {
    ...
    well_info = NULL
    warning(paste('Well Information Section not recognised in file', lasfile))
  }
  ...
  if (length(sections$curves) > 0) {
    ...
    curves = NULL
    warning(paste('Curve Information Section not recognized in file', lasfile))
  }
  ...
  if (length(sections$log_data) > 0) {
    ...
    log_data = NULL
    warning(paste('ASCII Log Data Section not recognised in file', lasfile))
  }
  ...
  if (length(sections$parameters) > 0) {
    ...
    parameters = NULL
    warning(paste('Parameter Information Section not recognised in file', lasfile))
